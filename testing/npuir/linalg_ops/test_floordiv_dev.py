...
def generate_tensor(shape, dtype, clear=False, positive=False):
    ...
    if dtype in ("float32", "float16"):
        ...
        t = torch.randint(-100, 100, size=shape, dtype=torch.int32)
        if positive:
            t = torch.abs(t) + 1
        return t
...
def test_floordiv_dev(floordiv_case):
    ...
    if dtype == "int32":
        ref = torch.div(a.cpu(), b.cpu(), rounding_mode='trunc')
    else:
        ref = torch.floor(a.cpu() / b.cpu())
    compiled_kernel(a, b, c)

    assert torch.allclose(c.cpu().float(), ref.float(), rtol=1e-3, atol=1e-3)